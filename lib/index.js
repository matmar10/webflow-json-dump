...
module.exports = function(options, imports) {
  ...
  async function getPopulatedItems(items, collectionId, rootCollectionId) {
    ...
    let output = await Promise.map(items, (item) => getPopulatedItem(item, collectionId, rootCollectionId), { concurrency: 1 });
    ...
    if (options.map) {
      output = output.map(item => {
        ...
        oldKeys.forEach(srcProperty => objectMapper.setKeyValue(item, undefined));
