...
module.exports = function(options, imports) {
  ...
  async function wait() {
    return new Promise((resolve) => setTimeout(resolve, options.wait || 0));
  }
  ...
  function getSet(rawValue, fieldDef) {
  ...
  async function getFieldValue(item, fieldDef, rootCollectionId) {
    ...
    switch (fieldDef.type) {
      ...
      case 'Set':
        return getSet(rawValue, fieldDef);
      case 'Option':
        return getItemOptionValue(rawValue, fieldDef);
      default:
        return rawValue;
    }
  }
